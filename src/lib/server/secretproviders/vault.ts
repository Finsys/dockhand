...
import type { SecretProvider, TestConnectionResult } from './shared';

/**
 * HashiCorp Vault config.
 *
 * NOTE FOR COORDINATOR: this interface is *not* yet present in ./shared (only
 * the `'vault'` type tag is). It is defined here so the provider compiles in
 * isolation. During integration, move this to ./shared as `VaultConfig`, add it
 * to the `SecretProviderConfig` union, and switch the import below to
