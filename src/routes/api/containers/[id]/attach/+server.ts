/**
 * Container Attach API
 *
 * POST: Creates an attach stream for a running container
 * Returns connection info for WebSocket attachment to container streams
 */
...
export const POST: RequestHandler = async ({ params, request, cookies, url }) => {
	...
	try {
		...
		return json({ error: error.message || 'Failed to attach to container' }, { status: 500 });
	}
};
